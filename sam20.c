...
void incert_in_hesh(pElem* table, int m, char* name) /*функция добавления элемента в таблицу*/
{
	...
	new_node->link = table[indx];
	table[indx] = new_node;
}


int delete_from_hesh(pElem* table, int m, char* target) /*функция удаления элемента из таблицы*/
{
	int indx = hash_function(target, m);
	pElem p = table[indx];
	pElem prev = NULL;
	while (p != NULL)
	{
		...
		{
			if (prev == NULL) { table[indx] = p->link; }
...
int main()
{
	...
	scanf("%s", target);
	if (delete_from_hesh(table, m, target) == 1) { n -= 1; }
