...
void vstavka(int *s, int l, int *counter1, int *counter2) /*функция сортировки методом вставки*/
{
    *counter1 = 0;
    *counter2 = 0;
	for (int i = 1; i < l; i++)
...
void vstavka_modify (int *s, int l, int *counter1_mod, int *counter2_mod, int code) /*функция модифицированного метода вставки*/
{
	*counter1_mod = 0;
	*counter2_mod = 0;
	for (int i = 1; i < l; i++)
...
void sravnenie(int start, int kolvo)
{
    ...
    printf("--------------------------------------------------------------------------------\n");
    printf("Размер(N) | Бинарная: Присваивания / Сравнения | Обычная: Присваивания / Сравнения\n");
    printf("--------------------------------------------------------------------------------\n");
    ...
    for (int i = start; i <= kolvo+start; i++)
	{
        ...
        vstavka_modify(s1, i, &counter_mod1, &counter_mod2, 0);
        vstavka(s2, i, &counter_std1, &counter_std2);

        printf("%9d | %22d / %9d | %21d / %9d\n", 
               i, 
               counter_mod1, counter_mod2, 
               counter_std1, counter_std2);
...
int main()
{
	...
	else if (flag == 2)
	{
		...
		s2 = (int*) malloc(l1*sizeof(int)); /*выделение памяти под динамический массив*/
		...
		int counter1, counter2, counter_mod1, counter_mod2;
		vstavka_modify(s1, l1, &counter_mod1, &counter_mod2, 1);
		printf("для модифицированного алгоритма: %d %d\n\n", counter_mod1, counter_mod2);
		vstavka(s2, l1, &counter1, &counter2);
		printf("для стандартного алгоритма:      %d %d\n", counter1, counter2);
